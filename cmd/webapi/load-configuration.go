...
type WebAPIConfiguration struct {
	...
	Web struct {
		...
		ShutdownTimeout time.Duration `conf:"default:5s" yaml:"ShutdownTimeout"`
	} `yaml:"Web"`
...
func loadConfiguration() (WebAPIConfiguration, error) {
	...
	if err != nil && !os.IsNotExist(err) {
		...
		// Unmarshal into a temporary struct first so we can ensure the file
		// configuration overrides values parsed from env/flags/defaults.
		var fileCfg WebAPIConfiguration
		err = yaml.Unmarshal(yamlFile, &fileCfg)
		if err != nil {
			return cfg, fmt.Errorf("can't unmarshal config file: %w", err)
		}
		// Overwrite top-level configurable sections with values from file
		cfg.Web = fileCfg.Web
		cfg.DB = fileCfg.DB
