...
func run() error {
	...
	if err != nil {
		...
		if cfg.DB.CleanupOnExit {
			logger.Infof("!!! Database file will be removed on exit, CleanupOnExit flag True in demo/config.yml !!!")
		}
	}
	defer func() {
		...
		if cfg.DB.CleanupOnExit {
			if err := os.Remove(cfg.DB.Filename); err != nil {
				logger.WithError(err).Warning("failed to remove database file on exit")
			} else {
				logger.Info("removed database file on exit")
			}
		}
	}()
